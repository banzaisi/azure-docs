...
4. In **Settings**, you can optionally configure target site settings:
   ...
   - **Cache Storage accounts**: Site Recovery needs extra storage account called cache storage in the source region. All the changes happening on the source VMs are tracked and sent to cache storage account before replicating those to the target location. This storage account should be Standard performance tier and not replicated (LRS).
